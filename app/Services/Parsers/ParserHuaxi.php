...
class ParserHuaxi extends AbstractShopifyParser
{
		...
		*/
            ...
            '联系地址' => [
                'callback' => function ($items, $sourceHeaders) {
					...
					if ($zip_code >= 100 && $zip_code <= 116){
						$city = "台北市";
					} else if ($zip_code >= 200 && $zip_code <= 206) {
						$city = "‎基隆市";
					} else if ($zip_code >= 209 && $zip_code <= 212) {
						$city = "連江縣";
					} else if ($zip_code >= 207 && $zip_code <= 253) {
						$city = "新北市";
					} else if ($zip_code >= 260 && $zip_code <= 290) {
						$city = "宜蘭縣";
					} else if ($zip_code == 300) {
						$city = "新竹市";
					} else if ($zip_code >= 302 && $zip_code <= 315) {
						$city = "新竹縣";
					} else if ($zip_code >= 320 && $zip_code <= 338) {
						$city = "桃園市";
					} else if ($zip_code >= 350 && $zip_code <= 369) {
						$city = "苗栗縣";
					} else if ($zip_code >= 400 && $zip_code <= 439) {
						$city = "台中市";
					} else if ($zip_code >= 500 && $zip_code <= 530) {
						$city = "彰化縣";
					} else if ($zip_code >= 540 && $zip_code <= 558) {
						$city = "南投縣";
					} else if ($zip_code == 600) {
						$city = "嘉義市";
					} else if ($zip_code >= 602 && $zip_code <= 625) {
						$city = "嘉義縣";
					} else if ($zip_code >= 630 && $zip_code <= 655) {
						$city = "雲林縣";
					} else if ($zip_code >= 700 && $zip_code <= 745) {
						$city = "台南市";
					} else if ($zip_code >= 800 && $zip_code <= 852) {
						$city = "高雄市";
					} else if ($zip_code >= 880 && $zip_code <= 885) {
						$city = "澎湖縣";
					} else if ($zip_code >= 890 && $zip_code <= 896) {
						$city = "金門縣";
					} else if ($zip_code >= 900 && $zip_code <= 947) {
						$city = "屏東縣";
					} else if ($zip_code >= 950 && $zip_code <= 966) {
						$city = "臺東縣";
					} else if ($zip_code >= 970 && $zip_code <= 983) {
						$city = "花蓮縣";
					}
                    ...
                    foreach ($items as $item) {
						...
						if (stripos($item[$indexCountry], "COD") !== false){
							...
							$province = ((stripos($city . $item[$indexStreet], "縣") === false) && (stripos($city . $item[$indexStreet], "市") === false)) ? $item[$indexProvince] : "";
							
							//if (((stripos($item[$indexProvince], "區") !== false) && (stripos($item[$indexCity], "區") === false) && (stripos($item[$indexStreet], "區") === false)))
