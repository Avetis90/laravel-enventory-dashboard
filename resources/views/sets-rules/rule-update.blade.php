            ...
            <div class="col-md-12 offset-content">
                <div class="box box-primary">
                    <form class="form-horizontal"
                          action="{{ route('rule-update', [$set, $model]) }}"
                                ...
                                <div class="col-sm-4">
                                    <select name="converter_id" id="converter_id" class="form-control">
                                        ...
                                        @foreach ($converters as $item)
                                            <option value="{{ $item->id }}" {{ old('converter_id') ?: $model->converter_id === $item->id ? 'selected' : "" }}>{{ $prettyNames[$item->converter_type] }}</option>
                                        @endforeach
                                    </select>
